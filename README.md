...
```bash
pytest -q
# Expected after this upgrade: 165 passed
```
...
- The ARC path enables classification loss and ARC-specific candidate generation, including support-only symbolic color, local-pattern, feature-lookup, position-color, geometric, nested-ring, translation, and clipped long-horizon adaptation programs. These are executable DSL programs, not post-hoc result wrappers.
- Final ARC evaluation uses support cross-validation: for each held-out task, accepted programs are selected by leave-one-demonstration-out accuracy on that task's public training pairs, then applied to the held-out test input without using test labels. Low-confidence support-translation programs are penalized at deployment so they cannot win on weak support evidence alone.
...
| quick | `169` | `5` | `13` | `0.6445714286` | `0.9475510204` | `+0.3029795918` | `0.0 -> 0.2` |
| full | `185` | `8` | `18` | `0.6680555556` | `0.7827777778` | `+0.1147222222` | `0.0 -> 0.0` |

Interpretation: the operator/program RSI loop now shows a larger held-out cell-accuracy improvement on the public ARC-AGI-1 same-shape subset, including full mode, and smoke/quick mode reach nonzero exact-grid success on held-out tasks. It is still not an official ARC leaderboard result and does not yet show exact-grid success in full mode. In some modes symbolic programs can trade cross-entropy loss for better discrete grids, so cell and exact-grid accuracy are the primary proof metrics for this adapter.

Earlier multi-seed ARC measurements over seeds `42, 43, 44` before the latest translation-guard rerun:
...
### OMEGA-THDSE-inspired HDC/topological ARC upgrade

The ARC adapter now includes an OMEGA-THDSE-inspired high-dimensional structural path:

- `arc_hdc_signature.py` implements deterministic 10,000-dimensional FHRR-style phase signatures with bind/bundle operations.
...
| Seed | Baseline cell accuracy | Previous evolved cell accuracy | HDC/topological evolved cell accuracy | Latest guarded evolved cell accuracy | Previous exact | Latest exact |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
Full-mode seed 42 now improves after the support-translation deployment guard: baseline cell accuracy `0.6680555556`, previous evolved cell accuracy `0.7673611111`, latest guarded evolved cell accuracy `0.7827777778`, cell delta `+0.1147222222`, exact-grid accuracy `0.0 -> 0.0`.
